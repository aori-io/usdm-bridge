 ...
 *
 * Only the fields needed by the SDK are tracked: the Aori chain key (used in
 * quote request bodies and matched against the Aori chain registry), default
 * public RPC URLs (used as fallback when the integrator doesn't override), and
 * `blockTimeMs` (used to delay status polling after a native deposit).
 */

export interface SdkChainConfig {
  id: number;
  /** Aori chain key (sent in quote request bodies; matches `aori.getChain`). */
  key: string;
